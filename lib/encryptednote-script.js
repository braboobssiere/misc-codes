...
document.getElementById("exportButton").addEventListener("click", async () => {
    ...
    // Create a blob and download the data as a .txt file
...
document.getElementById("importButton").addEventListener("change", async (event) => {
    const file = event.target.files[0];
    if (file) {
        const reader = new FileReader();
        reader.onload = async (e) => {
            const data = new Uint8Array(e.target.result);
            document.getElementById("output").value = Array.from(data).map(b => String.fromCharCode(b)).join('');
        };
